...
###############  Connect to LSL Stream ############### # don't need the emg data during the JND task
...
def control_structure():
    ...
    for i, ref_angle in enumerate(reference_angles):
        test_angle = ref_angle + test_resolutions[i]
        ...
        participant_answer = input("Was the second angle larger, smaller, or identical to the first? Hit '+' for larger, '-' for smaller' or '=' for identical.")   
    ...
    with open(f"jndGuesses_{filename_timestamp}.txt", 'w') as guessFile:
        ...
        guessFile.write(f'Reference Angles: {str(reference_angles)}\n')
