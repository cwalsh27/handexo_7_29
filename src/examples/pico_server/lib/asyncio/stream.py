...
class Stream:
    ...
    async def readline(self):
        ...
        while True:
            ...
            _ = self.s.recv_into(msg, MAXBUF)
            l2 = ''.join([chr(b) for b in msg])
...
async def start_server(cb, host, port, backlog=5):
    ...
    #import usocket as socket                                # Jshulgach: usocket not supported by CircuitPython
    import wifi, socketpool                                  # Jshulgach: use these librarires instead 

    # Create and bind server socket.
    socket = socketpool.SocketPool(wifi.radio)               # Jshulgach: Create socket explicitely from SocketPool
    host = socket.getaddrinfo(host, port)[0]  # TODO this is blocking!
    #s = socket.socket()                                     
    s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)   # Jshulgach: protocol type needs to be established
    s.setblocking(False)
    #s.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1) # JShulgach: Disabled as enums not supported natively
    s.bind(host[-1])
    s.listen(backlog)
